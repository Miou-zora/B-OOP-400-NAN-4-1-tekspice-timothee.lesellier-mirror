...
nts::Tristate ntsNor(nts::Tristate v, nts::Tristate v2)
{
...
nts::Tristate nts::NorComponent::compute(std::size_t pin)
{
    if (pin != _pinMax) {
        ...
        return ntsNor(_links[1]->getComponent().compute(_links[1]->getOtherPin()), _links[2]->getComponent().compute(_links[2]->getOtherPin()));
    }
}
